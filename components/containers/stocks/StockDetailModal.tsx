...
import type { OHLCV, StockHistory, StockItem, StockPeriod } from '@/types/finance';
...
interface ChartDatum extends OHLCV {
  ...
  mid: number; // (high+low)/2 — used by line chart
}

type ChartType = 'line' | 'candle';

const PERIODS: StockPeriod[] = ['1w', '1m', '3m', '1y', '5y'];
const CHART_TYPES: ChartType[] = ['line', 'candle'];

const GAIN_COLOR = '#16a34a';
const LOSS_COLOR = '#dc2626';
const SYNC_ID = 'stockDetailChartSync';
...
function formatPriceCompact(n: number) {
  if (n >= 1000) return n.toLocaleString('en-US', { maximumFractionDigits: 0 });
  return n.toLocaleString('en-US', { maximumFractionDigits: 2 });
}

function formatVolumeCompact(n: number) {
...
function formatDateForPeriod(iso: string, period: StockPeriod) {
...
function ChartTooltip({ active, payload, labels, onHover, visible = true }: ChartTooltipProps) {
  // Notify parent of currently hovered datum so we can render the
  // Y-axis crosshair label outside Recharts.
  useEffect(() => {
  ...
  return (
    <div className={styles['stock-detail__tooltip']}>
      ...
      <div className={styles['stock-detail__tooltip-row']}>
        ...
        <span>${formatPrice(item.open)}</span>
      </div>
      <div className={styles['stock-detail__tooltip-row']}>
 ...
 *
 * Recharts gives us pixel coords (x, y, width, height) for a Bar whose
 * dataKey is the [low, high] tuple — that defines the wick range. We
 * compute open/close pixel positions by linear interpolation inside
 * that range and draw the body rectangle. Up candles use gain color,
 * down candles use loss color.
 */
...
export default function StockDetailModal({ messages, stock, onClose }: StockDetailModalProps) {
  ...
  // Hovered datum is only valid if it was captured under the current period+chartType.
  // This avoids a setState-in-effect reset when the user switches tabs.
  const hovered =
  ...
  // Lazy fetch history. Parent passes key={symbol} so this effect runs once per modal mount.
  useEffect(() => {
    let cancelled = false;
    const controller = new AbortController();

    fetch(`/api/stocks/${stock.symbol}/history`, { signal: controller.signal })
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
      .then((json: { data: StockHistory }) => {
        if (cancelled) return;
  ...
  // The reference value differs by chart type:
  //  - line mode tracks the mid (high+low)/2, so reference uses mid as well
  //  - candle mode tracks close, so reference uses close
      ...
      <div className={styles['stock-detail__modal']} ref={modalRef}>
        ...
        <header className={styles['stock-detail__header']}>
          ...
          <div className={styles['stock-detail__price-row']}>
            <span className={styles['stock-detail__price']}>${formatPrice(stock.price)}</span>
            <span className={`${styles['stock-detail__change']} ${changeClass}`}>
              {arrow} {sign}{formatPrice(periodChange)} ({sign}{periodChangePct.toFixed(2)}%)
            </span>
          </div>
        </header>

        {/* Controls: chart type toggle (left) + period tabs (right) */}
        ...
        {/* Charts: stacked price + volume, synced via syncId */}
        <div className={styles['stock-detail__chart']} aria-label={`${stock.symbol} price chart`}>
          ...
          {!loading && !error && chartData.length > 0 && (
            <>
              ...
              <ResponsiveContainer width="100%" height={280}>
                ...
                >
                  <defs>
                    <linearGradient id="stockDetailLineGrad" x1="0" y1="0" x2="0" y2="1">
                  ...
                  {hovered && hoveredCrosshairValue !== null && (
                    <ReferenceLine
                      ...
                      strokeWidth={1}
                      label={{
                        value: `$${formatPrice(hoveredCrosshairValue)}`,
                        position: 'insideLeft',
                        fill: '#ffffff',
                        fontSize: 11,
                        fontWeight: 600,
                  ...
                  {chartType === 'line' ? (
                    <>
                      ...
                      />
                      {/* Line over the mid value */}
                      <Area
                        type="monotone"
                        dataKey="mid"
                        stroke={accentColor}
                        strokeWidth={1.5}
                        fill="url(#stockDetailLineGrad)"
                        activeDot={{
                          r: 6,
                          fill: accentColor,
                          stroke: '#FFFFFF',
                    ...
                    /* Candlestick: Bar over [low, high] with custom shape */
        ...
        <div className={styles['stock-detail__stats']}>
          <div className={styles['stock-detail__stat']}>
            ...
            <span className={`${styles['stock-detail__stat-value']} ${changeClass}`}>
              ${last ? formatPrice(last.close) : formatPrice(stock.price)}
            </span>
          </div>
          <div className={styles['stock-detail__stat']}>
            <span className={styles['stock-detail__stat-label']}>{messages.periodOpen}</span>
            <span className={styles['stock-detail__stat-value']}>
              ${first ? formatPrice(first.close) : formatPrice(stock.price)}
            </span>
          </div>
          <div className={styles['stock-detail__stat']}>
            <span className={styles['stock-detail__stat-label']}>{messages.volume}</span>
            <span className={styles['stock-detail__stat-value']}>
              {formatVolume(last?.volume ?? stock.volume)} {messages.shares}
            </span>
          </div>
        </div>
      </div>
